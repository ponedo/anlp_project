...
from step9_get_sent_dataset import load_file_data_label


pipeline = load_stuff("clf_sent")
ranks = []
predictions = []
unpredicted = {}
...
def extract_key_sents(file_id):
    ...
    sents, data, _ = zip(*load_file_data_label(file_id))
    y_prob_ = pipeline.predict_proba(data)

    # data = pipeline.steps[0][1].transform(data)
    # y_prob_ = pipeline.steps[1][1].predict_proba(data.toarray())
    
    result_dict = {}
    for sent_id, _, prob in zip(sents, data, y_prob_):
        result_dict[sent_id] = prob[1]
